...
const BrainNote = ({ note }) => {
  ...
  return (
    <MDXProvider components={components}>
      <Helmet>
        <meta charSet="utf-8" />
        <title>{note.title} - akiffpremjee's notes</title>
      </Helmet>
      <div
        id="brainNote"
        className="container max-w-2xl px-4 mx-auto text-gray-900 flex flex-col min-h-screen"
      >
        <div className="font-bold py-2 border-b -mx-4 px-4">Akiff's notes</div>
        ...
        <div className="refs-box bg-indigo-100 text-gray-600 rounded-lg mb-4 p-4">
          ...
          <p className="text-sm m-0">
            ...
            <a href="https://twitter.com/messages/compose?recipient_id=428712846">message</a>{' '}
            on Twitter.
          </p>
        </div>
      </div>
